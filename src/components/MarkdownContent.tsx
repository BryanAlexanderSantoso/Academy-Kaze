...
const CodeBlock = ({ node, inline, className, children, ...props }: any) => {
  ...
  if (inline) {
    return (
      <code className="bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-lg font-bold text-sm" {...props}>
        {children}
      </code>
    );
  }
    ...
    <div className="prose max-w-none custom-markdown">
      <ReactMarkdown
        ...
        components={{
          code: CodeBlock,
          blockquote: ({ children }) => (
            <div className="my-10 p-8 bg-gradient-to-br from-gray-50 to-white border-l-8 border-red-400 rounded-r-[40px] flex gap-6 items-start relative overflow-hidden shadow-sm ring-1 ring-gray-100 ">
          ...
          h1: ({ children }) => (
            <div className="mt-16 mb-8 border-b border-gray-100 pb-6">
              <h1 className="text-4xl lg:text-5xl font-bold tracking-tighter uppercase text-gray-900 leading-none">
                {children}
              </h1>
            </div>
          ),
          h2: ({ children }) => (
            <h2 className="text-2xl lg:text-3xl font-bold tracking-tighter uppercase mt-12 mb-6 text-gray-800 flex items-center gap-4">
          ...
          li: ({ children }) => (
            <li className="flex items-start gap-4 text-gray-600 font-medium text-lg">
              <span className="w-2 h-2 rounded-full bg-indigo-400 mt-3 shrink-0"></span>
              <span>{children}</span>
            </li>
          ),
          strong: ({ children }) => (
            <strong className="font-bold text-gray-900 uppercase tracking-tight mx-1">
              {children}
            </strong>
          ),
          a: ({ href, children }) => (
            <a
              ...
              className="text-indigo-600 font-bold underline underline-offset-8 decoration-indigo-200 hover:decoration-indigo-600 transition-all "
            >
              {children}
            </a>
          )
