      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-[60] bg-white/80 backdrop-blur-2xl border-b border-gray-100 px-6 py-5 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <span className="font-bold text-[11px] uppercase tracking-tighter leading-none text-gray-900">Kaze Developer</span>
            <span className="text-[8px] font-bold uppercase tracking-widest text-indigo-600 mt-1 ">Platform Pembelajaran</span>
          </div>
        </div>
      ...
      <aside className="hidden lg:flex fixed left-0 top-0 bottom-0 w-[110px] bg-white border-r border-gray-100 flex-col items-center py-12 z-50 shadow-[20px_0_60px_-15px_rgba(0,0,0,0.03)]">
          ...
          {sidebarItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <div className="absolute left-full ml-10 px-6 py-3 bg-gray-900 text-white text-[10px] font-bold uppercase tracking-[0.2em] rounded-2xl opacity-0 translate-x-[-15px] group-hover:opacity-100 group-hover:translate-x-0 transition-all pointer-events-none whitespace-nowrap z-50 shadow-2xl ">
                  {item.label}
                </div>
              </button>
            );
        ...
        <div className="mt-auto flex flex-col gap-8">
          ...
          >
            ...
            <div className="absolute left-full ml-10 px-6 py-3 bg-red-600 text-white text-[10px] font-bold uppercase tracking-[0.2em] rounded-2xl opacity-0 translate-x-[-15px] group-hover:opacity-100 group-hover:translate-x-0 transition-all pointer-events-none whitespace-nowrap z-50 shadow-2xl ">
              Keluar
            </div>
          </button>
          ...
          <motion.div
            ...
            className={`w-14 h-14 rounded-[22px] flex items-center justify-center font-bold text-sm overflow-hidden cursor-pointer shadow-xl border-4 transition-all ${user?.is_premium ? 'bg-indigo-50 border-white text-indigo-600 ring-2 ring-indigo-100' : 'bg-gray-50 border-white text-gray-400 shadow-inner'}`}
          >
            ...
            >
              <div className="flex items-center justify-between mb-16">
                <div className="flex items-center gap-4">
                  ...
                  />
                  <div className="flex flex-col">
                    <span className="font-bold text-[12px] uppercase tracking-tighter leading-none text-gray-900">Kaze Developer</span>
                    <span className="text-[8px] font-bold uppercase tracking-widest text-indigo-600 mt-1">Siswa Dashboard</span>
                  </div>
                </div>
                <button onClick={() => setSidebarOpen(false)} className="p-3 bg-gray-50 rounded-xl text-gray-400 active:scale-90 transition-all">
                  <X size={22} />
                </button>
              </div>
              ...
              <div className="mt-8 space-y-4">
                <div className="p-6 bg-gray-50 rounded-[30px] border border-gray-100 flex items-center gap-4">
                  ...
                  <div className="flex-1 overflow-hidden">
                    <p className="font-bold text-[11px] uppercase tracking-widest text-gray-900 truncate leading-none">{user?.full_name}</p>
                    <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest mt-1.5 truncate">ID: {user?.id.slice(0, 12)}</p>
                  </div>
                </div>
                <button
                  onClick={handleSignOut}
                  className="w-full flex items-center justify-center gap-4 p-5 bg-red-50 text-red-600 rounded-xl font-bold text-[11px] uppercase tracking-[0.3em] active:scale-95 transition-all border border-red-100"
                >
          ...
          <div className="max-w-[1700px] mx-auto px-16 py-8 flex items-center justify-between">
            <div className="flex items-center gap-16">
              <div className="space-y-1">
                <h1 className="text-2xl font-bold tracking-tighter text-gray-900 uppercase leading-none">Dashboard</h1>
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1.5 px-3 py-1 bg-indigo-50 rounded-full border border-indigo-100">
                    ...
                    <p className="text-[9px] font-bold text-indigo-600 uppercase tracking-widest leading-none">Jalur: {user?.learning_path === 'fe' ? 'Frontend' : user?.learning_path === 'be' ? 'Backend' : user?.learning_path === 'seo' ? 'SEO' : 'Fullstack'}</p>
                  </div>
                  <p className="text-[8px] font-bold text-gray-300 uppercase tracking-[0.3em] leading-none ml-2">VERSI 2.0</p>
                </div>
              </div>
              ...
              <div className="relative group/search">
                ...
                <div className="relative flex items-center">
                  ...
                  <input
                    ...
                    className="bg-gray-50/50 border-2 border-transparent rounded-xl py-3 pl-12 pr-6 text-[11px] font-bold uppercase tracking-widest w-[260px] focus:w-[320px] focus:ring-0 focus:border-indigo-600/10 focus:bg-white transition-all shadow-inner placeholder:text-gray-300"
                  />
                  <div className="absolute right-3 top-1/2 -translate-y-1/2 px-2 py-0.5 bg-white border border-gray-100 rounded-md text-[7px] font-bold text-gray-300 uppercase tracking-widest hidden group-focus-within/search:block">CARI</div>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-10">
              <div className="flex items-center gap-4 border-r border-gray-100 pr-10">
                <NotificationBell />
                <button className="p-3 bg-gray-50 text-gray-300 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all active:scale-90 group/settings">
              ...
              <div className="flex items-center gap-6">
                <div className="flex flex-col items-end">
                  <p className="text-[10px] font-bold text-gray-900 uppercase tracking-widest leading-none">{user?.full_name}</p>
                  <p className="text-[7px] font-bold text-gray-400 uppercase tracking-widest mt-1 opacity-60">ID://{user?.id.slice(0, 12)}</p>
                </div>
                <div className={`relative px-4 py-2.5 rounded-xl text-[9px] font-bold uppercase tracking-[0.2em] border-2 transition-all flex items-center gap-2 ${user?.is_premium
                  ? 'bg-gradient-to-r from-amber-500/10 to-orange-500/10 text-amber-600 border-amber-200 shadow-sm'
                  : 'bg-gray-50 text-gray-400 border-gray-100 shadow-inner'
                  }`}>
                  {user?.is_premium && <Crown className="w-3 h-3 animate-pulse" />}
                  {user?.is_premium ? 'Premium' : 'Gratis'}
                </div>
                <button
                  onClick={handleSignOut}
                  className="p-3 bg-red-50 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition-all shadow-sm active:scale-95 group relative border border-red-100"
                >
                  <LogOut className="w-5 h-5" />
                </button>
              </div>
