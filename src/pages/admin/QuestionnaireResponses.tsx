  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-white">
        ...
        <p className="mt-6 text-gray-400 font-bold uppercase tracking-[0.3em] text-[10px] animate-pulse">Aggregating Audit Intelligence...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div className="max-w-md w-full text-center">
          ...
          <h3 className="text-2xl font-bold text-gray-900 uppercase ">Null Reference</h3>
          ...
          <button onClick={() => navigate('/admin/questionnaires')} className="mt-10 btn-primary px-10 py-4 rounded-2xl font-bold text-[10px] uppercase tracking-widest">
            Return to Control
          </button>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-8 py-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div className="flex items-center gap-6">
              ...
              <div>
                <h1 className="text-2xl font-bold tracking-tighter flex items-center gap-4 uppercase ">
                  <BarChart2 className="w-7 h-7 text-indigo-600" />
                  {questionnaire.title}
                </h1>
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="flex h-1.5 w-1.5 rounded-full bg-indigo-500"></span>
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Audit Terminal / Insight View</p>
                </div>
              </div>
            </div>
            <button onClick={exportToCSV} className="flex items-center gap-3 px-8 py-4 bg-gray-900 hover:bg-black text-white rounded-2xl text-[10px] font-bold uppercase tracking-[0.2em] shadow-xl shadow-gray-900/10 transition-all active:scale-95">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[
            ...
            >
              ...
              <div className="relative">
                <div className="text-3xl font-bold text-gray-900 tracking-tighter mb-1">{stat.value}</div>
        ...
        <div className="card bg-white border border-gray-100 rounded-[40px] p-8 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="relative flex-1 group">
            ...
            <input
              ...
              className="w-full bg-gray-50 border-none rounded-[24px] py-4 pl-14 pr-6 text-[10px] font-bold uppercase tracking-widest focus:ring-[12px] focus:ring-indigo-500/5 focus:bg-white transition-all shadow-inner placeholder:text-gray-300"
            />
          </div>
          <div className="flex bg-gray-50 p-1.5 rounded-[24px]">
            {(['all', 'submitted', 'pending'] as const).map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-10 py-3 text-[10px] font-bold uppercase tracking-widest rounded-[20px] transition-all ${filter === f
        ...
        <div className="space-y-6">
          {filteredResponses.length === 0 ? (
            <div className="card bg-white border-dashed border-2 py-32 text-center rounded-[50px]">
              ...
              <h3 className="text-xl font-bold text-gray-900 uppercase ">No Matches Detected</h3>
                  ...
                  <div className="flex flex-col lg:flex-row gap-12">
                    <div className="flex-1">
                      <div className="flex flex-wrap items-center gap-4 mb-6">
                        <div className="w-14 h-14 bg-indigo-50 rounded-[28px] flex items-center justify-center text-indigo-600 font-bold text-xl shadow-inner border border-indigo-100/50">
                          {response.student?.full_name?.charAt(0)}
                        </div>
                        <div>
                          <h3 className="text-2xl font-bold text-gray-900 tracking-tighter uppercase leading-none mb-2">
                            {response.student?.full_name}
                          </h3>
                          <div className="flex items-center gap-3">
                            <Mail className="w-3 h-3 text-gray-300" />
                            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{response.student?.email}</span>
                          </div>
                        </div>
                        <div className="ml-auto flex gap-2">
                          {response.submitted_at ? (
                            <span className="px-5 py-2 text-[8px] font-bold uppercase tracking-widest bg-emerald-50 text-emerald-600 rounded-full border border-emerald-100">SUBMITTED</span>
                          ) : (
                            <span className="px-5 py-2 text-[8px] font-bold uppercase tracking-widest bg-amber-50 text-amber-600 rounded-full border border-amber-100">SYNCHRONIZING</span>
                          )}
                          {response.is_graded && (
                            <span className="px-5 py-2 text-[8px] font-bold uppercase tracking-widest bg-indigo-600 text-white rounded-full shadow-lg shadow-indigo-600/20">VALIDATED</span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-6 mb-10 pb-10 border-b border-gray-50">
                        <div className="space-y-1">
                          <p className="text-[8px] font-bold text-gray-300 uppercase tracking-widest flex items-center gap-2">
                            <Calendar className="w-3 h-3" /> Timestamp
                          </p>
                          <p className="text-[10px] font-bold text-gray-600">{response.submitted_at ? new Date(response.submitted_at).toLocaleString() : 'N/A'}</p>
                        </div>
                        <div className="space-y-1">
                          <p className="text-[8px] font-bold text-gray-300 uppercase tracking-widest flex items-center gap-2">
                            <Clock className="w-3 h-3" /> Latency
                          </p>
                          <p className="text-[10px] font-bold text-gray-600">{response.time_spent_seconds ? `${Math.round(response.time_spent_seconds / 60)} minutes` : 'N/A'}</p>
                        </div>
                        <div className="space-y-1">
                          <p className="text-[8px] font-bold text-gray-300 uppercase tracking-widest flex items-center gap-2">
                            <Zap className="w-3 h-3 text-indigo-500" /> Performance
                          </p>
                          <p className="text-[10px] font-bold text-indigo-600">{response.score !== undefined ? `${response.score.toFixed(1)}%` : 'PENDING_AUDIT'}</p>
                        </div>
                      </div>
                      ...
                      {response.submitted_at && (
                        <div className="space-y-6">
                          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4">Transmission Payload Preview</p>
                            ...
                            {questionnaire.questions_json.map((question, idx) => {
                              ...
                              return (
                                <div key={question.id} className="group/item">
                                  <p className="text-[10px] font-bold text-indigo-400 uppercase tracking-tighter mb-2 group-hover/item:text-indigo-600 transition-colors">
                      ...
                      {response.feedback && (
                        <div className="mt-10 p-8 bg-indigo-50/50 border border-indigo-100 rounded-[32px] relative overflow-hidden group">
                          ...
                          <p className="text-[10px] font-bold text-indigo-600 uppercase tracking-widest mb-3">Audit Intelligence Feedback</p>
                          <p className="text-sm font-bold text-indigo-900 leading-relaxed relative z-10">"{response.feedback}"</p>
                        </div>
                      )}
                    </div>
                    ...
                    {response.is_graded && (
                      <div className="lg:w-96 flex-shrink-0 flex items-center justify-center border-l border-gray-50 pl-12 ">
                        <div className="text-center">
                          ...
                          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">RECORD_LOCKED</p>
...
const GradeForm: React.FC<{
  ...
  if (!showForm) {
    return (
      ...
      >
        ...
        <div>
          <p className="text-xl font-bold uppercase tracking-tighter">Initiate Audit</p>
          <p className="text-[10px] font-bold opacity-60 uppercase tracking-widest mt-1 text-center">Protocol Phase 3</p>
        </div>
      </button>
    );
  }
  ...
  return (
    ...
    >
      <h4 className="text-xl font-bold text-gray-900 mb-8 uppercase flex items-center justify-between">
        Validation Console
      ...
      <div className="space-y-10 flex-1">
        <div className="space-y-4">
          <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-2">Performance Score (0-100)</label>
          <div className="relative">
            <input
              type="number"
              value={score}
              onChange={(e) => setScore(parseFloat(e.target.value))}
              className="w-full bg-gray-50 border-none rounded-[28px] py-6 px-10 text-4xl font-bold text-indigo-600 focus:bg-white transition-all shadow-inner outline-none text-center"
              min="0"
              max="100"
              step="0.1"
            />
            <div className="absolute right-8 top-1/2 -translate-y-1/2 text-sm font-bold text-gray-300 pointer-events-none">%</div>
          </div>
        </div>

        <div className="space-y-4">
          <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-2">Intelligence Synthesis (Feedback)</label>
          <textarea
            value={feedback}
            onChange={(e) => setFeedback(e.target.value)}
            className="w-full bg-gray-50 border-none rounded-[32px] p-8 text-sm font-bold text-gray-700 focus:bg-white transition-all shadow-inner h-40 outline-none resize-none placeholder:text-gray-300 "
        ...
        <div className="space-y-4 pt-4">
          <button
            ...
            className="w-full py-6 bg-indigo-600 text-white font-bold uppercase tracking-[0.2em] text-[10px] rounded-[30px] shadow-xl shadow-indigo-600/20 hover:bg-indigo-700 hover:-translate-y-1 transition-all active:translate-y-0"
          >
            COMMIT_RECORD_TO_REPOSITORY
          </button>
          <button
            onClick={() => setShowForm(false)}
            className="w-full py-5 text-[10px] font-bold text-gray-300 hover:text-gray-900 uppercase tracking-widest transition-colors"
          >
            Abort Assessment
          </button>
        </div>
