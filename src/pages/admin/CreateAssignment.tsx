  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center">
        ...
        <p className="mt-6 text-gray-400 font-bold uppercase tracking-[0.3em] text-[10px] animate-pulse">Syncing Operative Registry...</p>
      </div>
    );
  }
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  ...
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Operation: Multi-Sync</p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-4 space-y-8">
              <div className="card bg-white border border-gray-100 rounded-[50px] p-10 sticky top-32 shadow-sm hover:shadow-2xl transition-all duration-700">
                <h2 className="text-xl font-bold text-gray-900 mb-10 flex items-center gap-4 uppercase ">
                ...
                <div className="space-y-10">
                  ...
                  <div className="space-y-4">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Target Knowledge Node</label>
                    <div className="relative">
                      <select
                        value={selectedCourse}
                        onChange={(e) => setSelectedCourse(e.target.value)}
                        className="w-full bg-gray-50 border-none rounded-[28px] py-5 px-8 text-xs font-bold uppercase tracking-widest focus:ring-[12px] focus:ring-indigo-500/5 focus:bg-white transition-all appearance-none cursor-pointer shadow-inner"
                        required
                      >
                        <option value="" className="text-gray-400">SELECT_NODE_ID</option>
                        {courses.map((course) => (
                          <option key={course.id} value={course.id} className="text-gray-900 font-bold">
                  ...
                  {/* Due Date */}
                  <div className="space-y-4">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Deadline Sequence</label>
                    <div className="relative">
                      <input
                        type="datetime-local"
                        value={dueDate}
                        onChange={(e) => setDueDate(e.target.value)}
                        className="w-full bg-gray-50 border-none rounded-[28px] py-5 px-8 text-[11px] font-bold uppercase tracking-widest focus:ring-[12px] focus:ring-indigo-500/5 focus:bg-white transition-all shadow-inner"
                      />
                  ...
                  <div className="p-8 bg-indigo-50 rounded-[40px] space-y-4 border border-indigo-100/50">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest">Total Population</span>
                      <span className="text-xl font-bold text-indigo-600 tracking-tighter">{students.length}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest">Marked for Sync</span>
                      <span className="text-3xl font-bold text-indigo-700 tracking-tighter">
                  ...
                  <div className="space-y-4 pt-4">
                    <button
                      ...
                      className="w-full py-6 bg-indigo-600 hover:bg-indigo-700 text-white font-bold uppercase tracking-[0.25em] text-[10px] rounded-[30px] shadow-2xl shadow-indigo-600/30 transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-4"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => navigate('/admin/assignments')}
                      className="w-full py-5 bg-white border border-gray-100 text-gray-400 hover:text-gray-900 font-bold uppercase tracking-[0.2em] text-[10px] rounded-[30px] transition-all"
                    >
                      Abort Operation
                    </button>
                  </div>
            ...
            <div className="lg:col-span-8 space-y-10">
              <div className="card bg-white border border-gray-100 rounded-[60px] p-12 shadow-sm hover:shadow-2xl transition-all duration-700">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
                  <h2 className="text-3xl font-bold text-gray-900 tracking-tighter flex items-center gap-5 uppercase ">
                  ...
                  <div className="relative group">
                    ...
                    <input
                      ...
                      className="bg-gray-50 border-none rounded-[20px] py-3.5 pl-12 pr-6 text-[10px] font-bold uppercase tracking-widest focus:ring-[12px] focus:ring-indigo-500/5 focus:bg-white transition-all w-full md:w-80 shadow-inner"
                    />
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-3 mb-12 pb-10 border-b border-gray-50">
                  {[
                    ...
                    <button
                      ...
                      className={`px-6 py-3 bg-${bulk.bg}-50 text-${bulk.bg}-600 hover:bg-${bulk.bg}-100 rounded-2xl text-[10px] font-bold uppercase tracking-widest transition-all active:scale-95`}
                    >
                      {bulk.label}
                    </button>
                  ))}
                ...
                {filteredStudents.length > 0 ? (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-3 mb-1">
                            <p className={`font-bold text-sm uppercase tracking-tight truncate ${selectedStudents.has(student.id) ? 'text-white' : 'text-gray-900'}`}>
                              {student.full_name}
                            </p>
                  ...
                  <div className="text-center py-24">
                    ...
                    <h3 className="text-xl font-bold text-gray-900 uppercase ">Repository Empty</h3>
