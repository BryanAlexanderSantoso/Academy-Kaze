...
const AdminAssignments: React.FC = () => {
  ...
  const getPathBadge = (path: string | undefined) => {
    ...
    return (
      <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-widest ${colors[path as keyof typeof colors]}`}>
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-gray-50">
        ...
        <p className="mt-4 text-gray-400 font-bold uppercase tracking-widest text-[10px] animate-pulse">Scanning Submissions...</p>
      </div>
    );
  }
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Audit Terminal</p>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-8">
          ...
          {[
            ...
            >
              ...
              <div className="flex items-end justify-between relative z-10">
                <span className="text-4xl font-bold text-gray-900 tracking-tighter">{stat.count}</span>
        ...
        <div className="bg-white rounded-[32px] shadow-sm border border-gray-100 overflow-hidden mb-8">
          <div className="px-8 py-5 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="p-1 bg-gray-50 rounded-2xl flex">
                {(['all', 'pending', 'graded', 'overdue'] as const).map((f) => (
                  <button
                    key={f}
                    onClick={() => setFilter(f)}
                    className={`px-6 py-2.5 text-[10px] font-bold uppercase tracking-widest rounded-xl transition-all ${filter === f
            ...
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Displaying {filteredAssignments.length} results</p>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50/50">
                  <th className="px-8 py-4 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest">Student / Trace</th>
                  <th className="px-8 py-4 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest">Assignment Module</th>
                  <th className="px-8 py-4 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest">Status / Timeline</th>
                  <th className="px-8 py-4 text-center text-[10px] font-bold text-gray-400 uppercase tracking-widest">Decision</th>
                </tr>
              </thead>
                    ...
                    >
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="text-sm font-bold text-gray-900">{(assignment as any).student?.full_name}</p>
                            <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <td className="px-8 py-6">
                        <div className="flex flex-col">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-bold text-gray-900">{(assignment as any).course?.title}</span>
                          </div>
                          {assignment.submission_link && (
                            <a
                              ...
                              className="flex items-center gap-2 mt-3 text-[10px] font-bold text-indigo-600 hover:text-indigo-800 uppercase tracking-widest bg-indigo-50 w-max px-3 py-1.5 rounded-lg transition-all"
                            >
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          {assignment.grade !== null ? (
                            <div className="flex items-center gap-2 text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-xl w-max border border-emerald-100">
                              ...
                              <span className="text-[10px] font-bold uppercase tracking-widest">Score: {assignment.grade}</span>
                            </div>
                          ) : assignment.submission_link ? (
                      ...
                      <td className="px-8 py-6 text-center">
                        {assignment.submission_link && !assignment.grade ? (
                          <button
                            ...
                            className="px-6 py-3 bg-indigo-600 text-white text-[10px] font-bold uppercase tracking-widest rounded-2xl shadow-xl shadow-indigo-600/20 hover:bg-indigo-700 transition-all active:scale-95"
                          >
                            Evaluate
                          </button>
                        ) : assignment.grade !== null ? (
                          ...
                          <span className="text-[10px] font-bold text-gray-200 uppercase tracking-widest">No Action</span>
                        )}
                      </td>
                    </motion.tr>
                  ))}
          ...
          {filteredAssignments.length === 0 && (
            <div className="py-20 text-center">
              ...
              >
                ...
                <h3 className="text-xl font-bold text-gray-900 mb-2 uppercase">Cache Empty</h3>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between mb-10">
                <div>
                  <h2 className="text-3xl font-bold text-gray-900 tracking-tight">Evaluate Task</h2>
              ...
              <div className="space-y-8">
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 ml-1">Performance Score (0-100)</label>
                  <div className="relative">
                    <input
                      type="number"
                      value={gradeForm.grade}
                      onChange={(e) => setGradeForm({ ...gradeForm, grade: e.target.value })}
                      className="w-full bg-gray-50 border-none rounded-[24px] p-6 text-4xl font-bold text-indigo-600 focus:bg-white transition-all shadow-inner text-center outline-none"
                      placeholder="--"
                    />
                ...
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 ml-1">Analytical Feedback</label>
                  <textarea
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button
                    onClick={() => setGradingId(null)}
                    className="py-5 bg-white border-2 border-indigo-50 text-indigo-600 font-bold uppercase tracking-widest text-[10px] rounded-[24px] transition-all"
                  >
                    Abort
                  </button>
                  <button
                    onClick={() => gradingId && handleGrade(gradingId)}
                    className="py-5 bg-indigo-600 text-white font-bold uppercase tracking-widest text-[10px] rounded-[24px] shadow-xl shadow-indigo-600/30 transition-all hover:bg-indigo-700 hover:-translate-y-1"
                  >
                    Commit Record
                  </button>
                </div>
