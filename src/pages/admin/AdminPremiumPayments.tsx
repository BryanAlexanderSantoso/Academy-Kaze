  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-white">
        ...
        <p className="mt-6 text-gray-400 font-bold uppercase tracking-[0.3em] text-[10px] animate-pulse">Initializing Financial Core...</p>
      </div>
    );
  }
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-50 shadow-sm">
        <div className="max-w-[1600px] mx-auto px-10 py-7 flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tighter text-gray-900 uppercase ">Financial Oversight</h1>
              <div className="flex items-center gap-2 mt-0.5">
                <span className="flex h-1.5 w-1.5 rounded-full bg-green-500 animate-pulse"></span>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none">Live Transaction Audit</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="relative group">
              ...
              <input
                ...
                className="bg-gray-100 border-none rounded-2xl py-3.5 pl-12 pr-6 text-[10px] font-bold uppercase tracking-widest focus:ring-[12px] focus:ring-indigo-500/5 focus:bg-white transition-all w-full md:w-80 shadow-inner placeholder:text-gray-400"
                value={searchTerm}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          {[
            ...
            >
              ...
              <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-10">{stat.label}</h3>
          ...
          <div className="px-10 py-8 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-8 bg-gray-50/30">
            <div className="flex items-center gap-10">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-bold text-gray-900 uppercase tracking-widest">Protocol Filter</span>
              </div>
              <div className="flex bg-white p-1.5 rounded-[22px] shadow-sm">
                {['all', 'pending', 'approved', 'rejected'].map((s) => (
                  <button
                    ...
                    className={`px-8 py-2.5 text-[9px] font-bold uppercase tracking-[0.15em] transition-all rounded-[18px] ${statusFilter === s
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-white text-left">
                  <th className="px-10 py-6 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Operative</th>
                  <th className="px-10 py-6 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Transaction Descriptor</th>
                  <th className="px-10 py-6 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Temporal Entry</th>
                  <th className="px-10 py-6 text-center text-[10px] font-bold text-gray-400 uppercase tracking-widest">Authorization</th>
                </tr>
              </thead>
                    ...
                    >
                      <td className="px-10 py-8">
                        <div className="flex items-center gap-5">
                          <div className="w-14 h-14 rounded-2xl bg-gray-50 border border-gray-100 flex items-center justify-center text-gray-400 font-bold text-lg group-hover:bg-indigo-600 group-hover:text-white group-hover:scale-110 transition-all duration-500 shadow-inner">
                            {(payment as any).user?.full_name?.charAt(0) || <Users className="w-5 h-5" />}
                          </div>
                          <div className="min-w-0">
                            <p className="text-sm font-bold text-gray-900 tracking-tight uppercase group-hover:text-indigo-600 transition-colors">{(payment as any).user?.full_name}</p>
                      ...
                      <td className="px-10 py-8">
                        <div className="flex flex-col gap-1.5">
                          <span className="text-xl font-bold text-gray-900 tracking-tighter">Rp {Number(payment.amount).toLocaleString()}</span>
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className={`text-[9px] font-bold px-2.5 py-1 rounded-lg uppercase tracking-widest border ${(payment as any).premium_type === 'premium_plus'
                            ...
                            {(payment as any).promo_code && (
                              <span className="text-[9px] font-bold text-purple-600 bg-purple-50 px-2 py-1 rounded-lg border border-purple-100 flex items-center gap-1">
                      ...
                      <td className="px-10 py-8">
                        <div className="flex flex-col gap-0.5">
                          <span className="text-[10px] font-bold text-gray-700 uppercase tracking-widest">{new Date(payment.created_at).toLocaleDateString()}</span>
                        ...
                        <div className="flex items-center justify-center gap-4">
                          {payment.status === 'pending' ? (
                            <button
                              ...
                              className="flex items-center gap-3 px-8 py-4 bg-gray-900 border border-black hover:bg-black text-white text-[10px] font-bold rounded-2xl shadow-xl shadow-gray-900/10 transition-all hover:scale-105 active:scale-95 uppercase tracking-widest"
                            >
                              REVIEW_CASE
                              <ArrowUpRight className="w-4 h-4 text-indigo-400" />
                            </button>
                          ) : (
                            <div className={`px-6 py-2 rounded-2xl text-[9px] font-bold uppercase tracking-[0.2em] border ${payment.status === 'approved'
            ...
            {filteredPayments.length === 0 && (
              <div className="py-32 text-center bg-white rounded-b-[50px]">
                ...
                >
                  ...
                  <h3 className="text-2xl font-bold text-gray-900 uppercase ">Repository Void</h3>
              ...
              <div className="md:w-[60%] bg-gray-50/50 p-12 flex flex-col relative overflow-hidden group">
                ...
                <div className="flex items-center justify-between mb-10 relative z-10">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="text-[10px] font-bold uppercase tracking-[0.3em] text-gray-400">Transaction Evidence</h4>
                ...
                <div className="flex-1 relative rounded-[45px] overflow-hidden bg-white shadow-2xl border border-gray-100 p-8 flex items-center justify-center group/img">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-gray-900/10 to-transparent flex items-end p-10 opacity-0 group-hover/img:opacity-100 transition-opacity">
                    <div className="w-full bg-white/20 backdrop-blur-md rounded-2xl p-4 border border-white/30 text-center">
                      <span className="text-[10px] font-bold text-white uppercase tracking-widest">ENHANCED_VIEW_READY</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="md:w-[40%] p-14 flex flex-col bg-white border-l border-gray-100 relative">
                <div className="flex items-center justify-between mb-12">
                  <div className="flex items-center gap-4">
                    ...
                    <h3 className="text-3xl font-bold text-gray-900 tracking-tighter uppercase ">Audit Console</h3>
                  </div>
                  <button
                    onClick={() => setSelectedPayment(null)}
                ...
                <div className="flex-1 space-y-12 overflow-y-auto pr-2 custom-scrollbar">
                  <section className="space-y-6">
                    <div className="p-8 bg-gray-50/50 rounded-[35px] border border-gray-100 relative overflow-hidden group/card shadow-inner">
                      <p className="text-[9px] font-bold text-gray-300 uppercase tracking-widest mb-4">Target Identity</p>
                      <div className="flex items-center gap-4 mb-2">
                        <div className="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center font-bold text-indigo-600">
                          {(selectedPayment as any).user?.full_name?.charAt(0)}
                        </div>
                        <p className="text-xl font-bold text-gray-900 uppercase">{(selectedPayment as any).user?.full_name}</p>
                      </div>
                    ...
                    <div className={`p-8 rounded-[35px] text-white relative overflow-hidden group shadow-2xl ${(selectedPayment as any).premium_type === 'premium_plus'
                      ...
                      <div className="relative z-10">
                        <div className="flex items-center justify-between mb-4">
                          <p className="text-[9px] font-bold uppercase tracking-widest opacity-60">Validation Amount</p>
                          <span className="px-3 py-1 bg-white/20 rounded-full text-[9px] font-bold uppercase tracking-wider flex items-center gap-1">
                            {(selectedPayment as any).premium_type === 'premium_plus' ? <><Crown className="w-3 h-3" /> PREMIUM+</> : <><Shield className="w-3 h-3" /> PREMIUM</>}
                          </span>
                        </div>
                        <p className="text-5xl font-bold tracking-tighter mb-2">Rp {Number(selectedPayment.amount).toLocaleString()}</p>
                        ...
                        {(selectedPayment as any).promo_code && (
                          <div className="mb-4 p-3 bg-white/10 rounded-xl flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-bold uppercase tracking-wider">Promo: {(selectedPayment as any).promo_code} (-{(selectedPayment as any).discount_percent}%)</span>
                          </div>
                        )}
                        <div className="flex gap-6 pt-6 border-t border-white/10">
                          <div>
                            <p className="text-[8px] font-bold uppercase mb-1 opacity-60">Method</p>
                            <p className="text-[10px] font-bold uppercase tracking-widest">{selectedPayment.payment_method}</p>
                          </div>
                          <div>
                            <p className="text-[8px] font-bold uppercase mb-1 opacity-60">Reference</p>
                            <p className="text-[10px] font-bold uppercase tracking-widest font-mono">{selectedPayment.transaction_id || 'NULL'}</p>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="space-y-4">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-4">Administrative Feedback Vector</label>
                    <textarea
                      className="w-full bg-gray-50 border-none rounded-[35px] p-8 text-sm font-bold text-gray-700 focus:bg-white transition-all h-36 outline-none resize-none shadow-inner placeholder:text-gray-300 "
                ...
                <div className="grid grid-cols-2 gap-6 mt-12 pt-10 border-t border-gray-50">
                  <button
                    ...
                    className="flex items-center justify-center gap-3 py-6 rounded-[30px] bg-white border-2 border-red-50 text-red-600 font-bold hover:bg-red-50 transition-all uppercase tracking-widest text-[10px] active:scale-95"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-3 py-6 rounded-[30px] bg-indigo-600 text-white font-bold hover:bg-indigo-700 transition-all shadow-2xl shadow-indigo-600/30 uppercase tracking-widest text-[10px] active:scale-95 hover:-translate-y-1"
                  >
