...
const AdminCourses: React.FC = () => {
  ...
  const getCategoryBadge = (category: string) => {
    ...
    return (
      <span className={`px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-widest ${colors[category as keyof typeof colors]}`}>
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-gray-50">
        ...
        <p className="mt-4 text-gray-400 font-bold uppercase tracking-widest text-[10px] animate-pulse">Compiling Catalog...</p>
      </div>
    );
  }
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Library Master</p>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-8">
        {filteredCourses.length === 0 ? (
          <div className="card py-32 text-center border-dashed border-2 bg-gray-50/50 rounded-[40px]">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-2 uppercase">Library Empty</h3>
            ...
            <Link to="/admin/courses/new" className="bg-indigo-600 text-white px-10 py-5 rounded-[24px] font-bold text-xs uppercase tracking-[0.2em] shadow-2xl shadow-indigo-600/30 hover:bg-indigo-700 transition-all inline-block">
                ...
                >
                  ...
                  <div className={`flex flex-col flex-1 ${viewMode === 'grid' ? 'p-8 pb-10' : 'px-8'}`}>
                    <div className="flex items-start justify-between mb-4">
                      <h3 className={`font-bold text-gray-900 group-hover:text-indigo-600 transition-colors ${viewMode === 'grid' ? 'text-2xl leading-tight' : 'text-lg'}`}>
                        {course.title}
                      </h3>
                    </div>
                    ...
                    <div className={`mt-8 pt-8 border-t border-gray-50 flex items-center justify-between ${viewMode === 'list' ? 'hidden' : 'block'}`}>
                      <Link
                        ...
                        className="flex items-center gap-2 text-[10px] font-bold text-indigo-600 uppercase tracking-[0.2em] group/link"
                      >
                        MANAGE CHAPTERS
