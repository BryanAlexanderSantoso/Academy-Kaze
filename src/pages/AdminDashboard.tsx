  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center">
        ...
        <p className="mt-6 text-gray-400 font-bold uppercase tracking-[0.3em] text-[10px] animate-pulse">Syncing Command Center...</p>
      </div>
    );
  }
      ...
      <div className="pl-24">
        ...
        <header className="bg-white/80 backdrop-blur-xl border-b border-gray-100 sticky top-0 z-40">
          <div className="max-w-[1600px] mx-auto px-10 py-7 flex items-center justify-between">
            <div>
              <h2 className="text-[10px] font-bold uppercase tracking-[0.4em] text-indigo-600 mb-1">Kaze For Developer</h2>
              <h1 className="text-2xl font-bold tracking-tighter text-gray-900 uppercase ">Intelligence Oversight</h1>
            </div>
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-4">
                <div className="text-right hidden sm:block">
                  <p className="text-xs font-bold uppercase tracking-widest text-gray-900">Chief Admin</p>
                ...
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-indigo-700 p-[1px] shadow-lg shadow-indigo-500/10">
                  <div className="w-full h-full rounded-[15px] bg-white flex items-center justify-center font-bold text-indigo-600">AD</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
            {[
              ...
              >
                ...
                <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-8">{stat.label}</h3>
                <div className="flex items-end gap-3 mb-2">
                  <span className="text-5xl font-bold tracking-tighter text-gray-900">{stat.value}</span>
                  <div className="mb-2 px-1.5 py-0.5 bg-indigo-50 text-indigo-600 rounded text-[10px] font-bold uppercase tracking-widest">+4.2%</div>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
            ...
            <div className="lg:col-span-8 space-y-16">
              <section>
                <div className="flex items-center justify-between mb-10">
                  <h2 className="text-2xl font-bold tracking-tight flex items-center gap-4 uppercase text-gray-900">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                  {quickActions.map((action, i) => (
                    ...
                    >
                      ...
                      <p className="font-bold text-sm uppercase tracking-[0.1em] text-gray-900 mb-1">{action.title}</p>
              ...
              <section>
                <div className="flex items-center justify-between mb-10">
                  ...
                  <Link to="/admin/assignments" className="px-5 py-2.5 bg-gray-100 hover:bg-gray-200 text-[10px] font-bold text-gray-500 hover:text-gray-900 rounded-xl transition-all uppercase tracking-widest">
                    Data Log
                  </Link>
                </div>
                <div className="space-y-4">
                  {recentAssignments.slice(0, 5).map((assignment, i) => (
                    ...
                    >
                      <div className="w-14 h-14 rounded-2xl bg-indigo-50 border border-indigo-100 flex items-center justify-center font-bold text-sm text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-sm">
                        {assignment.grade ? assignment.grade : <Zap className="w-6 h-6 animate-pulse" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-1">
                          <h3 className="font-bold text-base uppercase tracking-tight truncate text-gray-900 group-hover:text-indigo-600 transition-colors uppercase">{assignment.course?.title}</h3>
                          <span className="px-2 py-0.5 bg-gray-100 text-[8px] font-bold text-gray-400 rounded uppercase tracking-widest">LOG_CH</span>
                        </div>
                        <p className="text-xs font-bold text-gray-400 uppercase tracking-widest truncate overflow-visible">Operative {assignment.student?.full_name}</p>
                      </div>
                      <div className="text-right flex flex-col items-end">
                        <p className="text-[10px] font-bold text-gray-300 uppercase tracking-widest mb-1 leading-none">Status</p>
                        {assignment.grade !== null ? (
                          <span className="text-[10px] font-bold text-emerald-600 uppercase bg-emerald-50 px-2 py-1 rounded">VALIDATED</span>
                        ) : (
                          <span className="text-[10px] font-bold text-amber-600 uppercase bg-amber-50 px-2 py-1 rounded animate-pulse">PENDING_SYNC</span>
                        )}
                      </div>
                    </motion.div>
                  ))}
            ...
            <div className="lg:col-span-4 space-y-16">
              <section>
                  ...
                  {students.slice(0, 6).map((student, i) => (
                    ...
                    >
                      <div className="flex items-center gap-5 mb-5">
                        <div className="w-12 h-12 rounded-2xl bg-indigo-50 flex items-center justify-center font-bold text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-inner">
                          {student.full_name.charAt(0)}
                        </div>
                        <div className="flex-1 min-w-0">
                          <h4 className="font-bold text-sm uppercase tracking-tight truncate text-gray-900 group-hover:text-indigo-600 transition-colors">{student.full_name}</h4>
                          <p className="text-[10px] font-bold text-gray-400 truncate uppercase tracking-widest">{student.email}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-[8px] font-bold text-indigo-600 uppercase tracking-widest mb-1 bg-indigo-50 px-1.5 py-0.5 rounded">{student.learning_path || 'NULL'}</p>
                          <Crown className={`w-3.5 h-3.5 ml-auto transition-colors ${student.is_premium ? 'text-yellow-500' : 'text-gray-200'}`} />
                        </div>
                      </div>
                      <div className="space-y-3">
                        <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400">
                          <span>Sync Progress</span>
                          <span className="text-gray-900 font-bold">{student.progress_percentage}%</span>
                        </div>
                ...
                <button className="w-full mt-8 py-5 bg-white border border-gray-100 hover:bg-gray-50 text-[10px] font-bold text-gray-400 hover:text-gray-900 rounded-[28px] transition-all uppercase tracking-[0.3em] shadow-sm">
              ...
              <section className="bg-indigo-600 p-10 rounded-[50px] shadow-2xl shadow-indigo-600/30 overflow-hidden relative">
                ...
                <h3 className="text-[10px] font-bold uppercase tracking-[0.3em] text-indigo-200 mb-8 flex items-center gap-3">
                ...
                <div className="space-y-5 font-mono text-[11px] text-white">
                  <div className="flex justify-between items-center bg-white/10 px-4 py-3 rounded-2xl">
                    <span className="opacity-60 uppercase font-bold tracking-widest">Database Uplink</span>
                    <span className="font-bold">STABLE</span>
                  </div>
                  <div className="flex justify-between items-center bg-white/10 px-4 py-3 rounded-2xl">
                    <span className="opacity-60 uppercase font-bold tracking-widest">Neural Cluster</span>
                    <span className="font-bold tracking-tighter">OPTIMAL_98.2%</span>
                  </div>
                  <div className="flex justify-between items-center bg-white/10 px-4 py-3 rounded-2xl">
                    <span className="opacity-60 uppercase font-bold tracking-widest">Security Core</span>
                    <span className="font-bold text-emerald-300">SHIELDED</span>
                  </div>
                  <div className="flex justify-between items-center bg-white/10 px-4 py-3 rounded-2xl">
                    <span className="opacity-60 uppercase font-bold tracking-widest">Encryption Load</span>
                    <span className="font-bold">0.02ms_LAT</span>
                  </div>
                </div>
              </section>
            </div>
