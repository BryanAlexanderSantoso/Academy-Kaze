  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh]">
        ...
        <p className="mt-6 text-gray-400 font-bold uppercase tracking-[0.3em] text-[10px] animate-pulse">Memuat Daftar Kuis...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-16">
      <div className="flex flex-col items-start gap-4">
        ...
        <p className="text-gray-400 text-lg font-medium ">Uji pemahaman Anda melalui kuis teknis dan survei interaktif.</p>
      </div>
      ...
      {questionnaires.length > 0 ? (
              ...
              >
                <div className="space-y-6">
                  <div className="flex flex-wrap items-center gap-3">
                    <span className={`px-4 py-1.5 text-[9px] font-bold uppercase tracking-widest rounded-xl border transition-all ${status.color === 'green' ? 'bg-green-50 text-green-700 border-green-100' :
                    ...
                    {questionnaire.time_limit_minutes && (
                      <span className="px-4 py-1.5 text-[9px] font-bold bg-purple-50 text-purple-700 rounded-xl border border-purple-100 flex items-center gap-1.5 uppercase tracking-widest">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-gray-900 group-hover:text-indigo-600 transition-colors uppercase tracking-tighter leading-tight mb-2">
                      {questionnaire.title}
                    </h3>
                    {questionnaire.description && (
                      <p className="text-gray-500 text-sm font-medium line-clamp-2">{questionnaire.description}</p>
                    )}
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-gray-50 rounded-[25px] border border-gray-100 space-y-1">
                      <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Pertanyaan</p>
                      <p className="text-lg font-bold text-gray-900 tracking-tighter">{questionnaire.questions_json?.length || 0} Item</p>
                    </div>
                    <div className="p-4 bg-gray-50 rounded-[25px] border border-gray-100 space-y-1">
                      <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Sisa Percobaan</p>
                      <p className="text-lg font-bold text-gray-900 tracking-tighter">{attemptsUsed} / {questionnaire.max_attempts}</p>
                    </div>
                  </div>

                  {questionnaire.due_date && (
                    <div className="flex items-center gap-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest bg-gray-50 p-4 rounded-2xl border border-gray-100">
                  ...
                  {latestResponse?.score !== undefined && (
                    <div className="bg-indigo-600 p-6 rounded-[30px] text-white shadow-xl shadow-indigo-600/20 relative overflow-hidden">
                      ...
                      <div className="flex items-center justify-between relative z-10">
                        <div>
                          <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-indigo-200">Skor Terakhir</p>
                          <p className="text-3xl font-bold tracking-tighter">{latestResponse.score.toFixed(1)}%</p>
                        </div>
                        {latestResponse.feedback && (
                          <div className="max-w-[150px] text-right">
                            <p className="text-[8px] font-bold line-clamp-2 text-indigo-100 opacity-80">"{latestResponse.feedback}"</p>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex gap-4">
                    {status.status === 'completed' ? (
                      <button
                        ...
                        className="w-full bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold py-5 rounded-[25px] flex items-center justify-center gap-3 transition-all transform hover:-translate-y-1 active:scale-95 uppercase tracking-[0.3em] text-[10px]"
                      >
                        <Eye className="w-4 h-4" />
                        LIHAT HASIL
                      </button>
                    ) : status.status === 'overdue' ? (
                      <button disabled className="w-full bg-gray-100 text-gray-300 font-bold py-5 rounded-[25px] flex items-center justify-center gap-3 opacity-50 cursor-not-allowed uppercase tracking-[0.3em] text-[10px]">
                      ...
                      <button
                        onClick={() => navigate(`/dashboard/questionnaires/${questionnaire.id}/take`)}
                        className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-5 rounded-[25px] flex items-center justify-center gap-3 transition-all transform hover:-translate-y-1 active:scale-95 shadow-xl shadow-indigo-600/30 uppercase tracking-[0.3em] text-[10px]"
                      >
                        <Play className="w-4 h-4" />
                        {status.status === 'in_progress' ? 'LANJUTKAN' : 'MULAI KUIS'}
                      </button>
                    )}
                  </div>
                </div>
        ...
        <div className="bg-white border border-gray-100 rounded-[60px] p-40 text-center group">
          ...
          <h3 className="text-3xl font-bold text-gray-900 uppercase tracking-tighter">Belum Ada Kuis</h3>
          <p className="text-gray-400 font-medium mt-4 max-w-sm mx-auto">Kuis evaluasi akan muncul secara otomatis di sini saat instruktur telah merilis materi evaluasi baru.</p>
        </div>
      )}
    </div>
  );
