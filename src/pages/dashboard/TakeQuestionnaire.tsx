  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh]">
        ...
        <p className="mt-6 text-gray-400 font-bold uppercase tracking-[0.3em] text-[10px] animate-pulse">Menyiapkan Kuis...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div className="bg-white p-12 rounded-[50px] shadow-2xl border border-gray-100 text-center max-w-md">
          ...
          <h3 className="text-2xl font-bold text-gray-900 mb-2 uppercase tracking-tighter">Kuis Tidak Tersedia</h3>
          <p className="text-gray-500 mb-8">Maaf, kuis ini belum tersedia atau sudah ditutup.</p>
          <button onClick={() => navigate('/dashboard/questionnaires')} className="w-full bg-indigo-600 text-white font-bold py-4 rounded-2xl uppercase tracking-[0.2em] text-[10px] hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-600/30">
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center p-6 relative overflow-hidden">
        ...
        >
          ...
          <h2 className="text-3xl font-bold text-gray-900 mb-4 uppercase tracking-tighter leading-none">KUIS SELESAI!</h2>
          <p className="text-gray-500 mb-10 leading-relaxed font-medium">
            Jawaban Anda telah berhasil dikirim dan dicatat oleh sistem. {questionnaire.show_correct_answers
              ? 'Anda dapat meninjau hasilnya di daftar kuis.'
              : 'Instruktur akan segera meninjau dan menilai jawaban Anda.'}
          </p>
          {existingResponse?.score !== undefined && (
            <div className="bg-indigo-600 p-8 rounded-[40px] mb-10 text-white shadow-2xl shadow-indigo-600/20">
              <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-indigo-300 mb-2">Skor Akhir</p>
          ...
          <button
            ...
            className="w-full bg-gray-900 hover:bg-black text-white font-bold py-5 rounded-[25px] flex items-center justify-center gap-3 transition-all transform hover:-translate-y-1 active:scale-95 shadow-xl shadow-gray-900/10 uppercase tracking-[0.3em] text-[10px]"
          >
            KEMBALI
        ...
        <div className="max-w-5xl mx-auto px-10 py-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-center gap-6">
              ...
              <div>
                <h1 className="text-lg font-bold text-gray-900 uppercase tracking-tighter leading-none">{questionnaire.title}</h1>
                <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mt-1">Ujian Aktif: Pertanyaan {currentQuestionIndex + 1} dari {questionnaire.questions_json.length}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-8">
              {timeRemaining !== null && (
                <div className={`flex items-center gap-3 px-6 py-3 rounded-2xl border transition-all ${timeRemaining < 300
                  ...
                  <span className="font-bold tracking-tight text-lg">{formatTime(timeRemaining)}</span>
                </div>
              )}
      ...
      <div className="max-w-4xl mx-auto px-10 py-20">
            ...
            <div className="relative z-10 space-y-10">
              <div className="space-y-4">
                <div className="flex items-center gap-4">
                  <span className="px-5 py-2 bg-indigo-600 text-white rounded-2xl text-[10px] font-bold uppercase tracking-widest shadow-lg shadow-indigo-600/20 ">
                ...
                <h1 className="text-4xl font-bold text-gray-900 leading-tight uppercase tracking-tighter">
                  {currentQuestion.question}
                </h1>
                {currentQuestion.description && (
                  <p className="text-gray-500 font-medium text-lg leading-relaxed">{currentQuestion.description}</p>
                )}
              </div>
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {currentQuestion.options?.map((option) => (
                      ...
                      >
                        ...
                        <span className="text-lg font-bold uppercase tracking-tighter text-gray-900 leading-none">{option.text}</span>
                        <input
                          type="radio"
                ...
                {currentQuestion.type === 'short_answer' && (
                  <div className="relative group">
                    <input
                      ...
                      onChange={(e) => handleAnswer(currentQuestion.id, e.target.value)}
                      className="w-full bg-gray-50 border-2 border-gray-50 focus:border-indigo-600/30 rounded-[30px] py-8 px-10 text-xl font-bold uppercase tracking-tighter text-gray-900 transition-all outline-none focus:ring-[15px] focus:ring-indigo-600/5 placeholder:text-gray-200"
                ...
                {currentQuestion.type === 'long_answer' && (
                  <div className="relative group">
                    <textarea
                      value={answers[currentQuestion.id] || ''}
                      onChange={(e) => handleAnswer(currentQuestion.id, e.target.value)}
                      className="w-full bg-gray-50 border-2 border-gray-50 focus:border-indigo-600/30 rounded-[40px] py-10 px-10 text-xl font-bold uppercase tracking-tighter text-gray-900 transition-all outline-none focus:ring-[20px] focus:ring-indigo-600/5 min-h-[300px] placeholder:text-gray-200 leading-relaxed"
                      placeholder="Masukkan jawaban lengkap Anda..."
                    />
                  </div>
                )}
                ...
                {currentQuestion.type === 'linear_scale' && (
                  <div className="py-10 px-10 bg-gray-50 rounded-[40px] border border-gray-100 shadow-inner">
                    <div className="flex items-center justify-between mb-10 px-4">
                      <span className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] ">{currentQuestion.minLabel || 'Sangat Kurang'}</span>
                      <span className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] ">{currentQuestion.maxLabel || 'Sangat Baik'}</span>
                    </div>
                    <div className="flex flex-wrap gap-4 justify-center">
                      {Array.from(
                        ...
                        <button
                          ...
                          className={`w-16 h-16 rounded-[22px] font-bold text-xl tracking-tighter transition-all transform hover:scale-110 ${answers[currentQuestion.id] === value
        ...
        <div className="flex items-center justify-between mt-16 pb-20">
          <button
            ...
            className="p-5 bg-white border border-gray-100 rounded-2xl flex items-center gap-4 text-[10px] font-bold text-gray-400 uppercase tracking-[0.3em] transition-all hover:bg-gray-50 hover:text-gray-900 disabled:opacity-30 disabled:cursor-not-allowed group active:scale-95"
          >
            <ArrowLeft className="w-5 h-5 group-hover:-translate-x-1 transition-transform" />
            SEBELUMNYA
          </button>
          ...
          <div className="flex gap-4">
            {currentQuestionIndex < questionnaire.questions_json.length - 1 ? (
              <button
                ...
                className="p-5 bg-indigo-600 text-white rounded-2xl flex items-center gap-4 text-[10px] font-bold uppercase tracking-[0.3em] transition-all hover:bg-indigo-700 shadow-xl shadow-indigo-600/30 hover:-translate-y-1 active:scale-95 group"
              >
                BERIKUTNYA
              ...
              <button
                ...
                disabled={submitting}
                className="p-5 bg-emerald-600 text-white rounded-2xl flex items-center gap-4 text-[10px] font-bold uppercase tracking-[0.3em] transition-all hover:bg-emerald-700 shadow-xl shadow-emerald-500/30 hover:-translate-y-1 active:scale-95 group"
              >
        ...
        <div className="mt-8 bg-gray-50 p-10 rounded-[50px] border border-gray-100">
          <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.3em] mb-8 flex items-center gap-3 ">
          ...
          <div className="flex flex-wrap gap-4">
            {questionnaire.questions_json.map((q, idx) => (
              <button
                ...
                className={`w-14 h-14 rounded-2xl font-bold text-sm tracking-tighter transition-all transform hover:scale-110 ${idx === currentQuestionIndex
