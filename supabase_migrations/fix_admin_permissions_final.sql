...
BEGIN
    ...
    ----------------------------------------------------------------
    -- This 'SECURITY DEFINER' function runs with the privileges of the creator (postgres/admin)
    -- This allows checking the admin role WITHOUT triggering the RLS on the profiles table again.
    
    CREATE OR REPLACE FUNCTION public.is_admin()
    RETURNS BOOLEAN 
    LANGUAGE plpgsql 
    SECURITY DEFINER 
    SET search_path = public -- Ensure search path is safe
    AS $$
    BEGIN
        RETURN EXISTS (
            SELECT 1
    ...
    -- Grant execute to authenticated users (so they can call it in policies)
    GRANT EXECUTE ON FUNCTION public.is_admin() TO authenticated;


    ----------------------------------------------------------------
    -- 2. RESET & FIX RLS for 'premium_payments'
    ----------------------------------------------------------------
    -- Ensure table exists
    ...
    RAISE NOTICE 'Successfully updated all permissions with is_admin() security definer.';
END $$;
